...
2. **Baseline reconstruction:** regenerate a normalized field-level table from existing Gemini 2.0 Flash outputs and verify the paper headline metrics before making new figures.
...
4. **Modern-model replication:** run a fixed, intentionally small perturbation set on newer Gemini and Qwen-class models. Do not retune transforms per model in the first pass; transfer of the original perturbation policy is itself part of the test.
